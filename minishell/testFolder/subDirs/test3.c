...
void encryptFiles(const char* file, bool recursive, bool verbose, bool mode, bool stats, bool threading)
{
    ...
    if(files != NULL  && files->head != NULL)
    {
        ...
        if(inputBuffer[0] != '\0' && (inputBuffer[0] == 'y' || inputBuffer[0] == 'Y'))
        {
            ...
            if(!threading) for(int i = 0; i < MAX_THREADS - 1; i++) sem_wait(&threadSemaphore);
            struct timeval startTime, stopTime;
            ...
            if(!threading) for(int i = 0; i < MAX_THREADS - 1; i++) sem_post(&threadSemaphore);    
    ...
    if(files != NULL  && files->head != NULL)
    {
        ...
        if(inputBuffer[0] != '\0' && (inputBuffer[0] == 'y' || inputBuffer[0] == 'Y'))
        {
			...
			while(curFile != NULL)
            {
                ...
                args->threadNum = i;
                // In case we break before finding false, e.g. no threading
